...
router.post("/add", (req, res) => {
  ...
  if (time < 0) {
    pool.query(
      "INSERT INTO metrics (uuid, appLaunches, timeSpent) VALUES(?, ?, ?) ON DUPLICATE KEY UPDATE appLaunches=appLaunches +1",
      [uuid, 1, 0],
      callback
    );
  } else {
    pool.query(
      "UPDATE metrics SET timeSpent=timeSpent+? WHERE uuid=?",
      [time, uuid],
      callback
    );
  }
